...
Comms::Comms( FOCMotor &_motor, Telemetry &_telem, SST::TCtr _ctr, SST::TCtr _interval ) 
    : te1(TIMEOUT1_SIG, this),
    motor(_motor),
    telem(_telem),
    ctr(_ctr),
...
void Comms::init(SST::Evt const * const ie) {
    (void)ie;
    te1.arm(ctr, interval);
}

void Comms::dispatch(SST::Evt const * const ie) {
    (void)ie;
}
