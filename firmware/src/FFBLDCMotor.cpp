...
// FFBLDCMotor( int pp , float R)
// - pp            - pole pair number
// - R             - motor phase resistance
// - KV            - motor kv rating (rmp/v)
// - L             - motor phase inductance
FFBLDCMotor::FFBLDCMotor(int pp, float _R, float _KV, float _inductance)
: FOCMotor()
{
...
*/
void FFBLDCMotor::linkDriver(BLDCDriver* _driver) {
  driver = _driver;
}

// init hardware pins
void FFBLDCMotor::init() {
...
int FFBLDCMotor::alignCurrentSense() {
...
// - to the index
int FFBLDCMotor::absoluteZeroSearch() {
...
void FFBLDCMotor::loopFOC() {
  ...
  float vel_FF_current = friction * feed_forward_velocity; // * something
  float accel_ff_current = inertia * feed_forward_accel;

  float current_sp_wFF = current_sp + vel_FF_current + accel_ff_current;

  // PMSM decoupling control and BEMF FF
  float VqFF_decoupling = 0;
  float VdFF_decoupling = 0;
  if(_isset(phase_resistance) && _isset(phase_inductance))  {
    VqFF_decoupling =  current_sp_wFF * phase_resistance + voltage_bemf;
    VdFF_decoupling = -current_sp_wFF * shaft_velocity * pole_pairs * phase_inductance;
  } else {
    VqFF_decoupling = current_sp;
  }
  ...
  switch (torque_controller) {
    ...
    case TorqueControlType::dc_current:
      ...
      voltage.q = PID_current_q(current_sp_wFF - current.q);
      voltage.d = 0; // handling the decoupling elsewhere;
    case TorqueControlType::foc_current:
      ...
      current.d = ABcurrent.alpha * ct + ABcurrent.beta * st;
      current.q = ABcurrent.beta * ct - ABcurrent.alpha * st;

      // filter values
      current.q = LPF_current_q(current.q);
      current.d = LPF_current_d(current.d);

      Vbemf.q = (voltage.q - phase_resistance * current.q) * Ts * phase_inductance * (current.q - current_prev.q);
      Vbemf.d = (voltage.d - phase_resistance * current.d) * Ts * phase_inductance * (current.d - current_prev.d);
      VqFF_decoupling =  current_sp_wFF * phase_resistance + Vbemf.q; // calculate with bemf est from current sense, rather than noisy encoder
  ...
  Uq =  _constrain(voltage.q + VqFF + VqFF_decoupling, -voltage_limit, voltage_limit);
  Ud = _constrain(voltage.d + VdFF + VdFF_decoupling, -voltage_limit, voltage_limit);

  // here we ditch all modulation except SVPWM and do the inverse park inline
  // in order to avoid an extra call the _sincos

  // Inverse park transform
  Ualpha =  ct * Ud - st * Uq;  // -sin(angle) * Uq;
  Ubeta =  st * Ud + ct * Uq;    //  cos(angle) * Uq;
...
void FFBLDCMotor::move(float new_target) {
  ...
  if (_isset(KV_rating)) {
    ...
    if (current_sense) {
      float shaft_velocity_est = Vbemf.q * (KV_rating*_SQRT3)/_RPM_TO_RADS;
      shaft_velocity = (shaft_velocity+shaft_velocity_est)/2;
    }
  }
  ...
  switch (controller) {
    case MotionControlType::torque:
      if(torque_controller == TorqueControlType::voltage){ 
        current_sp = 0; // if voltage torque control
      } else {
    ...
    case CustomMCType::angle_nocascade:
      shaft_angle_sp = target;
      // calculate velocity set point
...
void FFBLDCMotor::setPhaseVoltage(float Uq, float Ud, float angle_el) {
...
float FFBLDCMotor::velocityOpenloop(float target_velocity){
...
float FFBLDCMotor::angleOpenloop(float target_angle){
  // get current timestamp
  unsigned long now_us = _micros();
  // calculate the sample time from last call
  float Ts = (now_us - open_loop_timestamp) * 1e-6f;
